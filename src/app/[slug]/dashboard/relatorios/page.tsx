...
export default function RelatoriosPage() {
  ...
  const tabClass = (tab: string) => `pb-3 px-6 text-sm font-bold uppercase tracking-widest transition-colors relative cursor-pointer ${activeTab === tab ? 'text-ruby' : 'text-zinc-400 hover:text-zinc-600'}`;

  return (
    <div className="p-8 w-full h-full animate-in fade-in duration-500 overflow-y-auto font-sansation">
      <div className="flex justify-between items-center mb-8 print:hidden">
        <div><h1 className="text-3xl font-bold tracking-tight text-zinc-900">Relatórios Gerenciais</h1><p className="text-zinc-500 mt-1 uppercase text-xs font-bold tracking-widest">Controle de equipe, bilheteria e histórico de ações.</p></div>
      ...
      {activeTab === 'staff' && (
        <div className="space-y-6 printable-area">
          <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm print:hidden">
            <label className="text-xs font-bold text-zinc-500 uppercase mb-2 block">Filtrar por Data do Evento</label>
          ...
          <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm">
            <Table><TableHeader className="bg-zinc-50"><TableRow className="not-italic"><TableHead className="font-bold text-zinc-900 uppercase text-xs">Funcionário</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Cargo</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Status Diária</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Valor</TableHead></TableRow></TableHeader>
              <TableBody>
                ...
                staffData.length === 0 ? <TableRow><TableCell colSpan={4} className="text-center py-12 text-zinc-400 uppercase text-xs font-bold">Nenhum staff escalado para esta data.</TableCell></TableRow> :
                staffData.map(s => <TableRow key={s.id} className="not-italic"><TableCell className="font-bold text-zinc-900">{s.employees?.nome}</TableCell><TableCell className="font-medium text-zinc-600">{s.employees?.cargo}</TableCell><TableCell><span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${s.tem_diaria ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-zinc-50 text-zinc-600 border border-zinc-100'}`}>{s.tem_diaria ? 'Sim' : 'Não'}</span></TableCell><TableCell className="font-bold text-zinc-900">{s.valor_diaria ? `R$ ${Number(s.valor_diaria).toFixed(2)}` : '—'}</TableCell></TableRow>)}
              </TableBody>
            </Table>
          </div>
        </div>
      ...
      {activeTab === 'guests' && (
        <div className="space-y-6 printable-area">
          <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm print:hidden">
            <label className="text-xs font-bold text-zinc-500 uppercase mb-2 block">Selecione o Evento</label>
          ...
          <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm">
            <Table><TableHeader className="bg-zinc-50"><TableRow className="not-italic"><TableHead className="font-bold text-zinc-900 uppercase text-xs">Convidado / Venda</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Tipo</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Qtd</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Check-in</TableHead></TableRow></TableHeader>
              <TableBody>
                {loading ? <TableRow><TableCell colSpan={4} className="text-center py-12"><Loader2 className="w-8 h-8 animate-spin mx-auto text-ruby" /></TableCell></TableRow> :
                guestData.length === 0 ? <TableRow><TableCell colSpan={4} className="text-center py-12 text-zinc-400 uppercase text-xs font-bold">Nenhum convidado ou venda registrada.</TableCell></TableRow> :
                guestData.map(g => <TableRow key={g.id} className="not-italic"><TableCell className="font-bold text-zinc-900">{g.name}</TableCell><TableCell className="text-xs font-bold text-zinc-500 uppercase">{g.event_benefits?.nome || 'NORMAL'}</TableCell><TableCell className="font-bold text-zinc-900">{g.quantity}</TableCell><TableCell><span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${g.checked_in ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'bg-zinc-50 text-zinc-500 border border-zinc-100'}`}>{g.checked_in ? 'ENTROU' : 'AUSENTE'}</span></TableCell></TableRow>)}
              </TableBody>
            </Table>
          </div>
        </div>
        ...
        <div className="space-y-6 printable-area">
          <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm print:hidden flex justify-between items-end">
            <div className="flex-1 max-w-md">
              <label className="text-xs font-bold text-zinc-500 uppercase mb-2 block">Buscar no Histórico</label>
            ...
            <p className="text-xs font-bold text-zinc-400 uppercase">Exibindo últimos 200 registros</p>
          </div>
          <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm">
            <Table><TableHeader className="bg-zinc-50"><TableRow className="not-italic"><TableHead className="w-48 font-bold text-zinc-900 uppercase text-xs">Data e Hora</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Usuário</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Ação</TableHead><TableHead className="font-bold text-zinc-900 uppercase text-xs">Tabela</TableHead></TableRow></TableHeader>
              <TableBody>
                {loading ? <TableRow><TableCell colSpan={4} className="text-center py-12"><Loader2 className="w-8 h-8 animate-spin mx-auto text-ruby" /></TableCell></TableRow> :
                filteredAudit.length === 0 ? <TableRow><TableCell colSpan={4} className="text-center py-12 text-zinc-400 uppercase text-xs font-bold">Nenhum registro de auditoria encontrado.</TableCell></TableRow> :
                filteredAudit.map(log => (
                  <TableRow key={log.id} className="not-italic hover:bg-zinc-50/50 transition-colors">
                    ...
                    <TableCell><span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${log.action.includes('EXCLUIU') ? 'bg-red-50 text-red-600 border border-red-100' : log.action.includes('CADASTROU') ? 'bg-green-50 text-green-600 border border-green-100' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>{log.action}</span></TableCell>
                    <TableCell className="text-xs font-bold text-zinc-400 uppercase">{log.target_table}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
