...
export default function RelatoriosPage() {
  ...
  const fetchEmployees = useCallback(async () => {
    if (!theaterId) return;
    const { data } = await supabase.from('employees').select('id, nome, cargo').eq('theater_id', theaterId).is('deleted_at', null).order('nome');
    setEmployees(data || []);
  ...
  return (
    <div className="p-8 w-full h-full animate-in fade-in duration-500 font-sans">
      <div className="flex flex-col items-center text-center md:text-left md:items-start md:flex-row justify-between mb-8 gap-6 print:hidden">
        ...
        <Button variant="outline" onClick={() => window.print()} className="cursor-pointer font-bold border-zinc-200 dark:border-white/10 dark:text-white rounded-xl h-11 px-6 shadow-sm"><Printer className="w-4 h-4 mr-2" />Imprimir Relatório</Button>
      </div>
