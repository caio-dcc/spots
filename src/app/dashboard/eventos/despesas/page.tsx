...
export default function DespesasAdicionaisPage() {
  ...
  const handleAddExpense = async () => {
    ...
    try {
      ...
      const { data: role } = await supabase.from('user_roles').select('organization_id').eq('user_id', (await supabase.auth.getUser()).data.user?.id).is('deleted_at', null).single();
      await logAction(role?.organization_id, 'ADICIONOU DESPESA', 'events', `${event?.title}: ${newDesc}`);
    } catch (error: any) {
