...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadOptions = async () => {
      ...
      const { data: emps } = await supabase.from('employees').select('id, nome').eq('user_id', userId).is('deleted_at', null).order('nome');
      const { data: evs } = await supabase.from('events').select('produtor, artistas').eq('user_id', userId).is('deleted_at', null);
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // 0. Pegar Organização do Usuário
      // 0. Pegar Usuário e Contexto
      const { getContextUserId } = await import("@/lib/auth-context");
      const userId = await getContextUserId();
      if (!userId) return;
      ...
      if (filters.employeeId || filters.artist || filters.producer) {
        let baseEq = supabase.from('events').select('id').eq('user_id', userId).is('deleted_at', null);
      ...
      // 1. Total Caixa Atual
      let guestsCurrentQ = supabase.from('guests').select('quantity, benefit_id, created_at, event_id, events!inner(ticket_price, user_id)').eq('events.user_id', userId).gte('created_at', startOfMonth).lte('created_at', endOfMonth);
      ...
      // Se tivermos filtro específico de evento, ignorar despesas globais
      if (!eventIdsFilter) {
        const { data: expenses } = await supabase.from('financial_transactions').select('amount').eq('user_id', userId).eq('type', 'EXPENSE').gte('transaction_date', startOfMonthDate).lte('transaction_date', endOfMonthDate);
        expensesCurrent = expenses?.reduce((acc: any, curr: any) => acc + curr.amount, 0) || 0;
        
        const { data: prevExpenses } = await supabase.from('financial_transactions').select('amount').eq('user_id', userId).eq('type', 'EXPENSE').gte('transaction_date', startOfPrevMonthDate).lte('transaction_date', endOfPrevMonthDate);
        prevExpensesTotal = prevExpenses?.reduce((acc: any, curr: any) => acc + curr.amount, 0) || 0;
      }

      // Adicionar Salários dos Efetivados (Despesa Fixa)
      const { data: contractedEmployees } = await supabase.from('employees').select('salary').eq('user_id', userId).eq('is_contracted', true).is('deleted_at', null);
      ...
      if (eventIdsFilter) {
        ...
        const { data } = await supabase.from('events').select('id, artistas').eq('user_id', userId).gte('event_date', startOfMonth).lte('event_date', endOfMonth).is('deleted_at', null);
        monthEventsData = data || [];
      }
      ...
      let guestsPrevQ = supabase.from('guests').select('quantity, benefit_id, event_id, events!inner(ticket_price, user_id)').eq('events.user_id', userId).gte('created_at', startOfPrevMonth).lte('created_at', endOfPrevMonth);
      ...
      // 3. Eventos e Visitas
      let evCurrQ = supabase.from('events').select('id, title, capacity, public_approval, event_date', { count: 'exact' }).eq('user_id', userId).is('deleted_at', null).gte('event_date', startOfMonthDate).lte('event_date', endOfMonthDate);
      if (eventIdsFilter) evCurrQ = evCurrQ.in('id', eventIdsFilter);
      const { data: eventsCurrent, count: eventCount } = await evCurrQ;

      let evPrevQ = supabase.from('events').select('id', { count: 'exact', head: true }).eq('user_id', userId).is('deleted_at', null).gte('event_date', startOfPrevMonthDate).lte('event_date', endOfPrevMonthDate);
      ...
      if (filters.employeeId) {
        ...
        const { count } = await supabase.from('employees').select('id', { count: 'exact', head: true }).eq('user_id', userId).eq('status', 'ativo').is('deleted_at', null);
        funcCount = count || 0;
      }
