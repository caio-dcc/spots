...
export function EventForm({ initialData, isEdit }: EventFormProps) {
  ...
  const [eventDate, setEventDate] = useState(() => {
    if (initialData?.event_date) {
      // Garantir que pegamos apenas YYYY-MM-DD para o input type="date"
      return String(initialData.event_date).split('T')[0];
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const eventPayload: any = {
        ...
        artistas: finalArtistas.length > 0 ? finalArtistas
          ...
          .map((a: any) => {
            ...
            return { nome: n, name: n, atracao: n, cache: c, fee: c, valor: c };
          }) : null,
      ...
      if (!isEdit) eventPayload.user_id = userId;
      ...
      try {
        await supabase.from('event_benefits').delete().eq('event_id', eventId);
        if (beneficios.length > 0) {
          const bensPayload = beneficios.filter(b => b.nome && b.valor_mask).map(b => ({
            event_id: eventId,
            nome: b.nome,
          ...
          if (bensPayload.length > 0) {
            ...
            if (insBenErr && insBenErr.code === 'PGRST204') {
              const fallbackPayload = bensPayload.map(({ quantity, ...rest }) => rest);
              await supabase.from('event_benefits').insert(fallbackPayload);
            } else if (insBenErr) throw insBenErr;
          }
        }
      } catch (benErr: any) { console.error("Erro ao sincronizar benefícios:", benErr.message); }
      ...
      <div className="max-w-6xl mx-auto">
            ...
            {isEdit && initialData?.id && (
              <div className="flex gap-4 mt-6">
                <Button
                  ...
                  variant="outline"
                  className="h-10 rounded-full border-ruby/20 text-ruby hover:bg-ruby hover:text-white transition-all font-bold text-xs"
                >
                  <Users className="w-4 h-4 mr-2" />
                  Gerar lista de convidados
                </Button>
                <Button 
                  variant="outline" 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto items-start">
          <div className="space-y-8 flex flex-col">
            <section className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 p-8 shadow-2xl">
              ...
              <div className="grid grid-cols-1 gap-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={`bg-zinc-50 dark:bg-white/5 h-14 rounded-2xl border-zinc-200 px-6 text-lg font-medium text-zinc-900 dark:text-white transition-all focus:ring-ruby focus:border-ruby ${getError('title') ? "border-red-500" : ""}`}
                    value={title}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input type="date" className="bg-zinc-50 dark:bg-zinc-900/50 h-14 rounded-2xl border-zinc-200 px-6 text-zinc-900 dark:text-white transition-all focus:ring-ruby shadow-sm" value={eventDate} onChange={e => setEventDate(e.target.value)} />
                  </div>
                  <div className="space-y-2">
              ...
              <div className="pt-6 border-t border-zinc-100 dark:border-white/5">
                ...
                {beneficios.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {beneficios.map(b => (
                      <div key={b.id} className="flex flex-col md:flex-row gap-3 items-stretch md:items-center bg-zinc-50 dark:bg-white/5 p-3 rounded-2xl border border-zinc-200 group animate-in slide-in-from-right-4">
                        <div className="flex-1">
                          <label className="text-[8px] font-black text-zinc-400 uppercase mb-1 block">Tipo do Ingresso</label>
                          <Input placeholder="Ex: Normal, VIP, APPAI..." className="bg-white dark:bg-white/5 h-12 rounded-xl w-full border-zinc-200 focus:ring-ruby font-bold text-zinc-900 dark:text-white" value={b.nome} onChange={e => setBeneficios(beneficios.map(ben => ben.id === b.id ? { ...ben, nome: e.target.value } : ben))} />
                        </div>
                        <div className="flex gap-3 items-end w-full md:w-auto">
                          <div className="w-24 md:w-20">
                            ...
                            <Input placeholder="0" type="number" className="bg-white dark:bg-white/5 h-12 w-full rounded-xl border-zinc-200 focus:ring-ruby text-center font-bold text-zinc-900 dark:text-white" value={b.quantidade} onChange={e => setBeneficios(beneficios.map(ben => ben.id === b.id ? { ...ben, quantidade: e.target.value } : ben))} />
                          </div>
                          <div className="w-32 md:w-32">
                            <label className="text-[8px] font-black text-zinc-400 uppercase mb-1 block">Preço (R$)</label>
                            <Input placeholder="0,00" className="bg-white dark:bg-white/5 h-12 w-full rounded-xl border-zinc-200 focus:ring-ruby font-mono font-bold text-right pr-4 text-zinc-900 dark:text-white" value={b.valor_mask} onChange={e => setBeneficios(beneficios.map(ben => ben.id === b.id ? { ...ben, valor_mask: maskCurrency(e.target.value) } : ben))} />
                          </div>
            ...
            <section className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 p-8 shadow-2xl">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <textarea className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 dark:bg-zinc-900/50 px-6 py-4 text-sm font-medium text-zinc-900 dark:text-white min-h-[160px] resize-none focus:ring-2 focus:ring-ruby outline-none transition-all shadow-sm" placeholder="Escreva sobre o espetáculo..." value={details} onChange={e => setDetails(e.target.value)} />
                </div>
              </div>
            </section>
          </div>
          ...
          <div className="space-y-8 flex flex-col">
                    ...
                    {funcionariosAssoc.map(f => (
                      <div key={f.id} className="p-4 bg-zinc-50 dark:bg-white/5 rounded-2xl border border-zinc-100 animate-in fade-in">
                        ...
                        <div className="flex items-center gap-4 pt-4 border-t border-zinc-200">
                          <div className="flex items-center bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 px-3 flex-1 shadow-sm">
            ...
            <section className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 p-8 shadow-2xl">
              ...
              <div className="space-y-6">
                <div className="bg-zinc-50 dark:bg-zinc-900/50 p-6 rounded-[1.5rem] border border-zinc-200 space-y-4 shadow-inner">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Nome do Artista</label>
                    <Input placeholder="Banda, DJ ou Atração..." className="bg-white dark:bg-zinc-900 h-14 rounded-xl text-base font-bold border-zinc-200 text-zinc-900 dark:text-white shadow-sm transition-all focus:ring-ruby" value={novoArtistaNome} onChange={e => setNovoArtistaNome(e.target.value)} />
                  </div>
                  <div className="flex gap-3">
                    <div className="flex-1 space-y-1">
                      <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Cachê Acordado</label>
                      <div className="flex items-center bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 px-4 shadow-sm h-14 transition-all focus-within:border-ruby focus-within:ring-4 focus-within:ring-ruby/10">
                ...
                {artistas.length > 0 && (
                  <div className="space-y-3 pt-4 border-t border-zinc-100 dark:border-white/5">
                    {artistas.map((a, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-transparent dark:bg-white/5 rounded-2xl border border-zinc-100 group animate-in slide-in-from-right-4">
            ...
            <section className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 p-8 shadow-2xl">
              ...
              <div className="space-y-4">
                <div className="space-y-1"><label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Som</label><Input placeholder="Técnico..." className="bg-zinc-50 dark:bg-zinc-900/50 h-11 rounded-xl text-xs font-bold border-zinc-200 text-zinc-900 dark:text-white" value={tecnicoSom} onChange={e => setTecnicoSom(e.target.value)} /></div>
                <div className="space-y-1"><label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Luz</label><Input placeholder="Iluminador..." className="bg-zinc-50 dark:bg-zinc-900/50 h-11 rounded-xl text-xs font-bold border-zinc-200 text-zinc-900 dark:text-white" value={tecnicoIluminacao} onChange={e => setTecnicoIluminacao(e.target.value)} /></div>
                <div className="space-y-1"><label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Produtor</label><Input placeholder="Nome..." className="bg-zinc-50 dark:bg-zinc-900/50 h-11 rounded-xl text-xs font-bold border-zinc-200 text-zinc-900 dark:text-white" value={produtor} onChange={e => setProdutor(e.target.value)} /></div>
              </div>
              ...
              <div className="space-y-4">
                {customFields.map((field, idx) => (
                  <div key={idx} className="flex flex-col gap-2 items-stretch animate-in slide-in-from-right-4 p-4 bg-zinc-50 dark:bg-white/5 rounded-2xl border border-zinc-100">
                    <Input 
                      placeholder="Título (Ex: Wi-fi, Camarim...)" 
                      className="bg-white dark:bg-zinc-950 h-12 rounded-xl w-full border-zinc-200 font-bold text-zinc-900 dark:text-white"
                      value={field.label}
                    ...
                    <div className="flex gap-2 items-center">
                      <Input 
                        ...
                        className="bg-white dark:bg-zinc-950 h-12 rounded-xl flex-1 border-zinc-200 font-medium text-zinc-900 dark:text-white"
                        value={field.value}
              ...
              <div className="space-y-4">
                {extraExpenses.map((exp, idx) => (
                  <div key={idx} className="flex flex-col gap-2 items-stretch animate-in slide-in-from-right-4 p-4 bg-zinc-50 dark:bg-white/5 rounded-2xl border border-zinc-100">
                    ...
                    />
                    <div className="flex gap-2 items-center">
                      <div className="flex items-center bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 px-4 flex-1 h-12">
          ...
          <DialogContent className="sm:max-w-[500px] bg-white dark:bg-zinc-950 border-0 shadow-2xl rounded-[2.5rem] p-8 font-sans">
            ...
            <div className="py-6 max-h-[400px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
              {dbFuncionarios.map(f => {
                ...
                return (
                  <div key={f.id} onClick={() => toggleFuncionario(f)} className={`flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer group ${isSelected ? 'bg-ruby/5 border-ruby/30' : 'bg-white dark:bg-zinc-900 border-zinc-100 hover:border-ruby/20'}`}>
